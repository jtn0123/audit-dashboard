...
const { buildRepoPosture, summarize } = require('./posture');
...
class Collector {
  ...
  async _refresh() {
    ...
    const postures = await mapLimit(selected, this.config.concurrency, repo => this.collectRepo(repo));
  ...
  async discoverRepos(errors) {
    ...
    if (this.config.owners.length) {
      for (const owner of this.config.owners) {
        try {
          // Try the org listing first; fall back to the user listing.
          push(await this.client.paginate(`/orgs/${owner}/repos?per_page=100&sort=pushed`, { maxPages: 5, allowStatus: [404] }));
        } catch (e) {
          if (e.status !== 404) errors.push({ scope: `org:${owner}`, message: e.message });
        }
        try {
          push(await this.client.paginate(`/users/${owner}/repos?per_page=100&sort=pushed`, { maxPages: 5, allowStatus: [404] }));
        } catch (e) {
  ...
  async collectRepo(repo) {
    ...
    const [config, alerts, pulls, dependabotRunAt, codeScanning] = await Promise.all([
    ...
    const posture = buildRepoPosture(raw, { staleDays: this.config.staleDays });
  ...
  getRepos({ filter, search, sort = 'risk' } = {}) {
    ...
    const filters = {
      all: () => true,
      attention: r => r.risk > 0,
      'no-dependabot': r => !r.dependabot.configPresent && !r.archived,
      'alerts-off': r => r.dependabot.alertsEnabled === false,
      vulnerable: r => r.alerts.counts.total > 0,
      critical: r => r.alerts.counts.critical > 0 || r.alerts.counts.high > 0,
      'open-prs': r => r.prs.counts.total > 0,
      'dependabot-prs': r => r.prs.counts.dependabot > 0,
      stale: r => r.lastScan.source === 'none' || (r.lastScan.ageDays != null && r.lastScan.ageDays > this.config.staleDays),
      clean: r => r.risk === 0
    };
    if (filter && filters[filter]) repos = repos.filter(filters[filter]);

    const sorters = {
      risk: (a, b) => b.risk - a.risk,
      name: (a, b) => a.fullName.localeCompare(b.fullName),
      alerts: (a, b) => b.alerts.counts.total - a.alerts.counts.total,
      prs: (a, b) => b.prs.counts.total - a.prs.counts.total,
      scan: (a, b) => (b.lastScan.ageDays ?? 1e9) - (a.lastScan.ageDays ?? 1e9),
      pushed: (a, b) => (a.pushedDaysAgo ?? 1e9) - (b.pushedDaysAgo ?? 1e9)
    };
    repos.sort(sorters[sort] || sorters.risk);
    return repos;
  }
...
module.exports = { Collector, mapLimit, parseDependabotConfig, rollupChecks };
