...
const { buildRepoPosture, appendCheckDerivedGaps, summarize, groupByAdvisory, classifyPr, parseBumpTitle } = require('./posture');
...
function normalizeSbomPackages(packages, cap = 3000) {
  ...
  for (const pkg of packages) {
    ...
    let ecosystem = null;
    let name = typeof pkg.name === 'string' ? pkg.name : '';
...
function repoFilter(filter, staleDays) {
  switch (filter) {
    ...
    case 'sla': return r => r.alerts.slaBreaches > 0;
    case 'secrets': return r => (r.secretAlerts || []).length > 0;
    case 'ci-failing': return r => r.ci?.state === 'failing';
...
class Collector {
    ...
    try {
      ...
      if (raw && Array.isArray(raw.repos)) {
        this.state = {
          ...
          packageIndex: this.config.collectSbom ? raw.packageIndex || null : null,
          fetchedAt: raw.fetchedAt
        };
        Object.assign(this.etags, raw.etags || {});
        // Secret-scanning responses can contain the exposed credential itself.
        // Remove bodies cached by earlier builds, including the disk copy.
        let scrubbed = false;
        for (const key of Object.keys(this.etags)) {
          if (key.includes('/secret-scanning/alerts')) { delete this.etags[key]; scrubbed = true; }
        }
        if (scrubbed) this.saveCache();
      }
  ...
  async _refresh() {
    ...
    this.state = {
      ...
      packageIndex: this.config.collectSbom && selected.length > 0 && postures.length === selected.length && this.packagesByRepo.size === selected.length
        ? buildPackageIndex(this.packagesByRepo) : null,
      errors,
    ...
    try { if (postures.length || !errors.length) this.history.record(this.state); } catch (e) { console.warn('[history] record failed:', e.message); }
    return this.state;
  }
  ...
  async collectRepo(repo) {
    ...
    const [
      config, alerts, pulls, dependabotRunAt, codeScanning, recentMerges,
      ...
      cfg.collectSbom ? this.fetchPackages(full, errors) : null
    ]);
    ...
    posture.packageCount = packages?.length ?? null;
    if (packages !== null) this.packagesByRepo.set(posture.fullName, packages);
      ...
      if (e.status === 403 || e.status === 404) {
        if (msg.includes('disabled')) {
          ...
          raw.alertsError = e.status === 403
            ? 'Token lacks access to Dependabot alerts (needs Dependabot alerts: Read permission)'
  ...
  async fetchSecretScanningAlerts(full, errors) {
    try {
      return await this.client.paginate(
        ...
        { maxPages: 2, allowStatus: [403, 404], useEtag: false }) || [];
    } catch (e) {
  ...
  searchPackages(query, { limit = 50 } = {}) {
    ...
    const repoCount = entry => new Set(entry.repos.map(r => r.repo)).size;
    const widestFirst = (a, b) => repoCount(b) - repoCount(a);
    const results = [...exact.sort(widestFirst), ...partial.sort(widestFirst)].slice(0, limit);
