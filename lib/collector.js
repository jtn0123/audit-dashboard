...
class Collector {
  ...
  getRepos({ filter, search, sort = 'risk' } = {}) {
    ...
    // Maps, not object literals: `filter` and `sort` come straight off the query
    // string, and `filters['constructor']` on a plain object resolves to an
    // inherited function that would silently pass every repo.
    ...
    const predicate = typeof filter === 'string' ? filters.get(filter) : undefined;
    ...
    const comparator = (typeof sort === 'string' && sorters.get(sort)) || sorters.get('risk');
