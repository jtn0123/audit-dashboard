...
function buildRepoPosture(raw, { staleDays = 14, stalePrDays = 14, now = Date.now() } = {}) {
  ...
  return {
    ...
    alerts: { counts: alertCounts, oldestOpenDays: alerts.reduce((max, a) => Math.max(max, a.ageDays ?? 0), 0) || null, list: alerts },
    prs: {
    ...
    gaps,
    risk,
    action: recommendedAction({ alertCounts, gaps, dependabotPrs, lastScan, config, alertsEnabled }),
    errors: raw.errors || []
  };
}

/** The single next thing to do for this repo — drives the "Action" column. */
function recommendedAction({ alertCounts, gaps, dependabotPrs, lastScan, config, alertsEnabled }) {
  ...
  if (!config.present) return { text: 'Add dependabot.yml', tone: 'warning' };
...
function summarize(repos, { staleDays = 14 } = {}) {
  ...
  return {
    ...
    reposNeedingAttention: repos.filter(r => r.risk > 0).length,
