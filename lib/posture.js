...
function parseBumpTitle(title = '') {
  // Dependabot writes two title shapes. The second was previously unparsed, so
  // every pip/"requirement" PR landed as bump: null and, downstream, as a
  // 'needs_human' verdict reading "unparseable bump":
  //
  //   bump express from 4.18.0 to 4.21.0
  //   update fakeredis requirement from >=2.37.0 to >=2.37.1
  //
  // The requirement form carries version *specifiers*, which may be several
  // comma-joined clauses (`<2.0,>=1.9.2`). semverBumpType resolves those to a
  // comparable version; here we only capture them.
  const bump = title.match(/bump\s+(\S+)\s+from\s+(\S+)\s+to\s+(\S+)/i);
  if (bump) return { package: bump[1], from: bump[2], to: bump[3] };

  const requirement = title.match(/update\s+(\S+)\s+requirement\s+from\s+(\S+)\s+to\s+(\S+)/i);
  if (requirement) return { package: requirement[1], from: requirement[2], to: requirement[3] };
...
/** A dismissed alert plus who dismissed it and why — the decision, not just the finding. */
function normalizeDismissedAlert(alert, now = Date.now()) {
  const base = normalizeAlert(alert, now);
  return {
    ...base,
    ...
    dismissedDaysAgo: daysSince(alert.dismissed_at, now)
  };
}

/** Code-scanning (CodeQL) alerts use their own severity vocabulary. */
function normalizeCodeScanningAlert(alert, now = Date.now()) {
  const rule = alert.rule || {};
  const raw = (rule.security_severity_level || rule.severity || 'low').toLowerCase();
  const severity = ['critical', 'high', 'medium', 'low'].includes(raw)
    ? raw
    : raw === 'error' ? 'high' : raw === 'warning' ? 'medium' : 'low';
  return {
    number: alert.number,
    url: alert.html_url,
    severity,
    rule: rule.id || rule.name || 'unknown',
    summary: rule.description || rule.name || 'Code scanning alert',
...
/** Secret-scanning alerts carry no severity: a leaked credential is a leaked credential. */
function normalizeSecretAlert(alert, now = Date.now()) {
  return {
    number: alert.number,
    url: alert.html_url,
...
const GAP_DEFS = {
  ...
  'sla-breach': { label: 'Past its patch budget', severity: 'critical', weight: 15, hint: 'An alert has been open longer than its severity allows' },
  'docker-uncovered': { label: 'Dockerfile not covered', severity: 'medium', weight: 8, hint: 'Add a "docker" ecosystem to dependabot.yml so base images get updates' },
  'ci-failing': { label: 'Default branch CI failing', severity: 'high', weight: 12, hint: 'Update PRs cannot merge safely while main is red' },
  'secret-alerts': { label: 'Open secret-scanning alerts', severity: 'critical', weight: 30, hint: 'Rotate the exposed credential, then close the alert' },
  'code-scanning-alerts': { label: 'Open code-scanning alerts', severity: 'high', weight: 10, hint: 'Triage the CodeQL findings' },
  'sonar-failing': { label: 'Sonar scan failing', severity: 'medium', weight: 0, hint: 'Usually an expired SONAR_TOKEN' }
};

/** Default age budget per severity, in days. Overridden by GH_SLA_*_DAYS. */
const DEFAULT_SLA = { critical: 7, high: 30, medium: 90, low: 180 };

/** The longest-open alert in a list, or null for an empty one. */
function oldestAlert(alerts) {
  if (!alerts.length) return null;
  return alerts.reduce((a, b) => ((a.ageDays ?? 0) >= (b.ageDays ?? 0) ? a : b), alerts[0]);
}

/**
 * Gaps that can only be known after check runs are attached — buildRepoPosture
 * runs before attachChecks, so the collector calls this afterwards. A failing
 * Sonar check on ANY open PR is a config problem (dead SONAR_TOKEN), not a
 * code problem, and it carries the two links that fix it.
 */
function appendCheckDerivedGaps(posture) {
  const prs = [...(posture.prs?.dependabot || []), ...(posture.prs?.other || [])];
  const sonarDown = prs.some(p => (p.checks?.failingNames || []).some(n => /sonar/i.test(n)));
  if (sonarDown && !posture.gaps.some(g => g.id === 'sonar-failing')) {
    posture.gaps.push({
      id: 'sonar-failing', ...GAP_DEFS['sonar-failing'], detail: null,
      links: [
        { label: 'new token', url: 'https://sonarcloud.io/account/security' },
        { label: 'repo secret', url: `https://github.com/${posture.fullName}/settings/secrets/actions` }
      ]
    });
  }
  return posture;
 ...
 */
function buildRepoPosture(raw, { staleDays = 14, stalePrDays = 14, sla = DEFAULT_SLA, now = Date.now() } = {}) {
  const repo = raw.repo || {};
  const alerts = (raw.alerts || []).map(a => normalizeAlert(a, now));
  const prs = (raw.pulls || []).map(p => normalizePr(p, now));

  // Mark each alert against its severity's age budget before counting them.
  for (const alert of alerts) {
    const budget = sla[alert.severity];
    alert.slaDays = budget ?? null;
    alert.breachesSla = budget != null && alert.ageDays != null && alert.ageDays > budget;
  }
  const breaches = alerts.filter(a => a.breachesSla);
  ...
  const dismissedAlerts = (raw.dismissedAlerts || []).map(a => normalizeDismissedAlert(a, now));
  const codeScanningAlerts = (raw.codeScanningAlerts || []).map(a => normalizeCodeScanningAlert(a, now));
  const secretAlerts = (raw.secretAlerts || []).map(a => normalizeSecretAlert(a, now));
  const ci = raw.ci || null;
  const dockerfiles = raw.dockerfiles || [];
  ...
  if (breaches.length) {
    const worst = oldestAlert(breaches);
    addGap('sla-breach', `${breaches.length} past budget · oldest ${worst.ageDays}d`);
  }
  // A container repo with no docker ecosystem never sees base-image updates,
  // however healthy its npm/pip config looks.
  const dockerEcosystems = (config.ecosystems || []).some(e => e.ecosystem === 'docker');
  if (dockerfiles.length && !dockerEcosystems && !repo.archived) {
    addGap('docker-uncovered', dockerfiles.length === 1 ? dockerfiles[0] : `${dockerfiles.length} Dockerfiles`);
  }
  if (ci?.state === 'failing') addGap('ci-failing', ci.workflow || ci.branch);
  if (secretAlerts.length) addGap('secret-alerts', `${secretAlerts.length} open`);
  if (codeScanningAlerts.some(a => a.severity === 'critical' || a.severity === 'high')) {
    addGap('code-scanning-alerts', `${codeScanningAlerts.length} open`);
  }
  ...
  return {
    ...
    gaps,
    risk,
    action: recommendedAction({ alertCounts, gaps, dependabotPrs, lastScan, alertsEnabled, secretAlerts, breaches }),
    errors: raw.errors || []
  };
}

/** The single next thing to do for this repo — drives the "Action" column. */
function recommendedAction({ alertCounts, gaps, dependabotPrs, lastScan, alertsEnabled, secretAlerts = [], breaches = [] }) {
  const has = id => gaps.some(g => g.id === id);
  // A live credential in a repo outranks everything else on this list.
  if (secretAlerts.length) {
    return { text: `Rotate ${secretAlerts.length} leaked secret${secretAlerts.length > 1 ? 's' : ''}`, tone: 'critical' };
  }
  if (alertsEnabled === false) return { text: 'Turn on Dependabot alerts', tone: 'critical' };
  if (breaches.length) {
    const worst = oldestAlert(breaches);
    return { text: `${breaches.length} past budget · oldest ${worst.ageDays}d`, tone: 'critical' };
  }
  ...
  if (has('ci-failing')) return { text: 'Fix the red default branch', tone: 'critical' };
...
function summarize(repos, { staleDays = 14 } = {}) {
  ...
  let slaBreaches = 0, secretAlerts = 0, codeScanningAlerts = 0, dismissedAlerts = 0;
  ...
  for (const r of active) {
    ...
    slaBreaches += r.alerts.slaBreaches || 0;
    secretAlerts += (r.secretAlerts || []).length;
    codeScanningAlerts += (r.codeScanningAlerts || []).length;
    dismissedAlerts += (r.dismissedAlerts || []).length;
  }
  ...
  return {
    ...
    codeScanningAlerts,
    dismissedAlerts,
    ciFailing: active.filter(r => r.ci?.state === 'failing').map(r => r.fullName),
    dockerUncovered: active.filter(r => r.gaps.some(g => g.id === 'docker-uncovered')).map(r => r.fullName),
...
/**
 * Pivot every open alert from repo-major to advisory-major.
 *
 * "GHSA-xxxx affects 4 repos, fixed in 4.17.21" is one decision; the same
 * finding spread across four repo rows reads as four unrelated chores.
 */
function groupByAdvisory(repos) {
  const byId = new Map();

  for (const repo of repos) {
    for (const alert of repo.alerts.list || []) {
      // GHSA is the stable identity; fall back to package+summary when absent.
      const id = alert.ghsaId || alert.cveId || `${alert.package}:${alert.summary}`;
      let entry = byId.get(id);
      if (!entry) {
        entry = {
          id,
          ghsaId: alert.ghsaId || null,
          cveId: alert.cveId || null,
      ...
      // Keep the worst severity seen; GitHub can report the same advisory
      // differently depending on how the dependency is reached.
      if (severityRank(alert.severity) < severityRank(entry.severity)) entry.severity = alert.severity;
      if (!entry.patchedVersion && alert.patchedVersion) entry.patchedVersion = alert.patchedVersion;
      entry.oldestDays = Math.max(entry.oldestDays, alert.ageDays ?? 0);
      if (alert.breachesSla) entry.breaches++;
      entry.repos.push({
        repo: repo.fullName,
        url: alert.url,
        version: alert.vulnerableRange,
        manifest: alert.manifest,
        ageDays: alert.ageDays,
        breachesSla: Boolean(alert.breachesSla)
      });
    }
  }

  return [...byId.values()]
    .map(a => ({ ...a, repoCount: new Set(a.repos.map(r => r.repo)).size }))
    .sort((a, b) =>
...
/** Lower is worse. Shared by the advisory sort and the UI. */
...
module.exports = {
  ...
  groupByAdvisory,
  severityRank,
  normalizeDismissedAlert,
  normalizeCodeScanningAlert,
  normalizeSecretAlert,
  GAP_DEFS,
  daysSince,
  newest,
  classifyPr,
