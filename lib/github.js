...
class GitHubClient {
  ...
  async request(pathOrUrl, { method = 'GET', accept = 'application/vnd.github+json', useEtag = true, allowStatus = [], retryOnLimit = true, body } = {}) {
    ...
    // Older persisted caches lack pagination metadata. Re-fetch them once so
    // a header-only 304 cannot silently truncate a multi-page response.
    if (cached?.etag && Object.hasOwn(cached, 'nextUrl')) headers['if-none-match'] = cached.etag;
    ...
    if (res.status === 304 && cached) {
      ...
      return { status: 200, data: cached.data, headers: res.headers, notModified: true,
        nextUrl: res.headers.get('link') == null ? cached.nextUrl : parseNextLink(res.headers.get('link')) };
    }

    // Checked before the rate-limit branch: a 403 usually means "this feature is
    // off for this repo", which callers ask for by name and shouldn't pay a retry for.
    ...
    if (res.status === 403 || res.status === 429) {
      ...
      // An absent header means unknown, not zero — Number(null) would say zero.
      const isLimit = retryAfter > 0 || (remainingHeader != null && Number(remainingHeader) === 0);
      if (isLimit && retryOnLimit) {
        const resetAt = Number(res.headers.get('x-ratelimit-reset')) * 1000;
        const waitMs = retryAfter > 0 ? retryAfter * 1000 : Math.max(0, resetAt - Date.now());
        if (waitMs <= 60_000) {
    ...
    if (useEtag && method === 'GET' && etag) this.etags[key] = { etag, data, nextUrl, usedAt: Date.now() };
    return { status: res.status, data, headers: res.headers, notModified: false, nextUrl };
  }
  ...
  async paginate(pathOrUrl, { maxPages = 5, allowStatus = [], accept, useEtag = true } = {}) {
    let url = pathOrUrl;
    const out = [];
    for (let page = 0; page < maxPages && url; page++) {
      const res = await this.request(url, { allowStatus, accept, useEtag });
...
/**
 * Keep the persisted ETag cache from growing without bound, evicting least
 * recently used first. Insertion order won't do: re-writing an existing key
 * keeps its original position, so hot entries would be dropped first.
 */
function pruneEtags(etags, maxEntries = 2000) {
  const keys = Object.keys(etags);
  if (keys.length <= maxEntries) return etags;
  const byAge = keys.sort((a, b) => (etags[a].usedAt || 0) - (etags[b].usedAt || 0));
  for (const key of byAge.slice(0, keys.length - maxEntries)) delete etags[key];
  return etags;
}
