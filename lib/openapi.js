  ...
  paths: {
    ...
    '/api/gh/repos': {
      get: {
        ...
        parameters: [
          { name: 'filter', in: 'query', schema: { type: 'string', enum: ['all', 'attention', 'no-dependabot', 'alerts-off', 'vulnerable', 'critical', 'open-prs', 'dependabot-prs', 'stale', 'clean', 'sla', 'secrets', 'ci-failing'] } },
    ...
    '/api/gh/advisories': { get: {
      summary: 'Open alerts grouped by advisory with affected repositories',
      parameters: [
        { name: 'severity', in: 'query', schema: { type: 'string', enum: ['critical', 'high', 'medium', 'low'] } },
        { name: 'minRepos', in: 'query', schema: { type: 'integer', minimum: 1 } }
      ],
      responses: { ...jsonResponse('Array of advisories, severity and blast radius'), ...NOT_CONFIGURED }
    } },
    '/api/gh/packages': { get: {
      summary: 'Cross-repository SBOM package search; exact names rank first',
      parameters: [{ name: 'q', in: 'query', schema: { type: 'string' } }],
      responses: { ...jsonResponse('{indexed, count, repoCount, results}'), ...NOT_CONFIGURED }
    } },
    '/api/gh/snapshots': { get: {
      summary: 'Flat scan projections for timeline and sparklines, oldest first; /history retains its existing contract',
      responses: { ...jsonResponse('Array of flat snapshots with at, alerts, coverage and byRepo'), ...NOT_CONFIGURED }
    } },
    '/api/gh/changes': { get: {
      summary: 'Changes since the last visit, or null when no baseline exists',
      parameters: [{ name: 'since', in: 'query', required: true, schema: { type: 'string', format: 'date-time' } }],
      responses: { ...jsonResponse('{changes}'), ...NOT_CONFIGURED, 400: { description: 'Missing or invalid timestamp' } }
    } },
