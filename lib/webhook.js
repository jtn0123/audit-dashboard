...
// A webhook body is attacker-supplied until proven otherwise. The repo name
// from it becomes a request path and a log line, so anything that isn't shaped
// like `owner/repo` is rejected here rather than sanitized downstream.
const SAFE_REPO = /^[\w.-]{1,100}\/[\w.-]{1,100}$/;

/** The repository a payload refers to, or null if it names none we can trust. */
function repoFromPayload(payload) {
  const full = payload?.repository?.full_name;
  return typeof full === 'string' && SAFE_REPO.test(full) ? full : null;
}
...
function planForEvent(event, payload) {
  ...
  if (!repo) return { action: 'ignore', reason: 'no usable repository in payload' };
...
function createWebhookHandler(collector, config) {
  return async function handleWebhook(req, res) {
    ...
    // GitHub's event names are lowercase words; anything else is not one of
    // ours and must not reach a log line or a response body verbatim.
    const raw = req.get('x-github-event') || '';
    const event = /^[a-z_]{1,40}$/.test(raw) ? raw : 'unknown';
    ...
    try {
      ...
      // Both values are validated above, and the format string is a literal —
      // a crafted header cannot forge a log line or smuggle a format directive.
      console.warn('[webhook] %s for %s failed: %s', event, plan.repo, e.message);
    }
  };
}
