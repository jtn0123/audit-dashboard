...
function groupByOverlap(prs) {
  ...
  if (unknowns.length && known.length) {
    const largest = known.reduce((a, b) => (b.members.length > a.members.length ? b : a), known[0]);
  ...
  for (const [repo, actions] of byRepo) {
    ...
    const trains = groupByOverlap(actions).map(train => {
      ...
      const steps = train.members.map((m, i) => {
        ...
        let why;
        if (prev == null) why = 'first in train — merge directly';
        else if (m.files == null) why = `files unknown — serialized conservatively after #${prev}`;
        else why = `shares ${shared[0] || 'files'} with the train — rebase after #${prev} lands`;
        return {
