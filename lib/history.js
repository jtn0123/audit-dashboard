...
const { legacySnapshot, canonicalSnapshot, changesSince } = require('./history-views');
...
class HistoryStore {
  ...
  load() {
    ...
    try {
      ...
      try {
        raw = JSON.parse(text);
        if (raw?.at && !Array.isArray(raw.snapshots)) raw = { snapshots: [canonicalSnapshot(raw)] };
      } catch {
        // Migrate the earlier JSONL format without discarding intact rows when
        // the last write was interrupted. Subsequent saves use one atomic file.
        raw = { snapshots: text.split('\n').flatMap(line => {
          try { return [canonicalSnapshot(JSON.parse(line))]; } catch { return []; }
        }) };
      }
  ...
  /** Flat timeline projections; the existing structured history API stays intact. */
  read() {
    return this.series().map(legacySnapshot);
  }

  append(row) {
    this.snapshots = compact([...this.snapshots, canonicalSnapshot(row)], {
      now: this.now(), retentionDays: this.retentionDays
    });
    this.save();
  }

  changesSince(since) {
    return changesSince(this.read(), since);
  }

  seriesFor(fullName) {
    return this.read().filter(row => fullName in row.byRepo)
      .map(row => ({ at: row.at, value: row.byRepo[fullName] }));
  }
...
module.exports = {
  HistoryStore, snapshotFrom, compact, diffSnapshots, MAX_SNAPSHOTS, DEFAULT_RETENTION_DAYS,
  History: HistoryStore,
  snapshot: (state, at) => legacySnapshot(snapshotFrom({ ...state, fetchedAt: at }))
};
