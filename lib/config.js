...
function loadConfig(env = process.env) {
  ...
  return {
    ...
    // Scan snapshots live beside the repo cache, on the same writable volume.
    historyFile: env.GH_HISTORY_FILE || path.join(
      path.dirname(env.GH_CACHE_FILE || path.join(__dirname, '..', '.cache', 'github.json')),
      'history.json'
    ),
    historyDays: Math.max(7, num(env.GH_HISTORY_DAYS, 180)),
    policyFile: env.PATCHBOARD_POLICY_FILE || path.join(__dirname, '..', '.patchboard-policy.yml'),
