 ...
 */
function buildActionQueue(repos, { stalePrDays = 14 } = {}) {
  const actions = [];
  ...
  for (const repo of repos) {
    ...
    for (const pr of repo.prs?.dependabot || []) {
      ...
      switch (v.verdict) {
        ...
        case 'needs_human':
        case 'no_ci':
          actions.push({ type: 'flag_pr', ...base, command: `gh pr view ${pr.number} ${gh}` });
          break;
...
module.exports = { bumpEcosystem, semverBumpType, cmpVersions, prVerdict, buildActionQueue, ACTION_RANK };
